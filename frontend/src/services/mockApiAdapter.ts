...
import { ISSUE_TYPES, ISSUE_STATUSES, IssueType, IssueStatus } from '../utils/constants';
...
const createMockApi = (api: AxiosInstance): MockAdapter => {
  ...
  mock.onGet(/\/issues\/\d+/).reply((config: any) => {
  ...
  mock.onPost('/issues').reply((config: any) => {
    try {
  ...
  mock.onPut(/\/issues\/\d+/).reply((config: any) => {
    try {
      const id = parseInt(config.url.split('/').pop()!);
  ...
  // Search
  mock.onGet(/\/issues\/search/).reply((config: any) => {
